...
## [1.6.12] — 2026-09-22 (วันลาของคนทำงานเป็นกะที่ยื่นล่วงหน้าเกินตารางกะ)

**พบ (ผล 123):** พนักงานที่มีกะวันเสาร์ 7 ครั้งตั้งแต่ ส.ค. ยื่นลาพักร้อน ศ. 16 – ส. 17 ต.ค. ระบบนับ 1 วัน เพราะตารางกะลงไว้ถึง 30 ก.ย. วันที่ไม่มีกะใช้กฎเสาร์-อาทิตย์หยุด และจำนวนวันถูกเก็บตอนยื่น ลงกะทีหลังก็ไม่เปลี่ยน — สิทธิ์ลาถูกหักขาด

### Fixed
- **บันทึกตารางกะเมื่อไหร่ ใบลาที่ยังรออนุมัติถูกคำนวณจำนวนวันใหม่** ด้วยกฎเดียวกับตอนยื่น (`recalcPendingLeaveDays`) · ใบที่อนุมัติแล้วไม่แตะ (มติ 19 ส.ค.: ไม่ปรับย้อนหลัง) · ลาครึ่งวันคง 0.5 · ลาเป็นชั่วโมงไม่แตะ · ช่วงที่กลายเป็นวันหยุดทั้งหมดไม่ถูกตั้งเป็น 0 · จด audit `leave-days-recalc` และแจ้งพนักงาน
- **ศูนย์อนุมัติขึ้นป้าย "⚠ ยังไม่ได้ลงกะ N วันในช่วงนี้"** กับใบลาของคนที่เคยมีกะทำงานวันเสาร์/อาทิตย์ ผู้อนุมัติจึงรู้ก่อนกดว่าจำนวนวันอาจไม่ถูก

### ใบลาที่พบ (LR_1789989492472_792)
ยังรออนุมัติ — HR ลงกะเดือน ต.ค. ของคนนี้ ใบลาจะปรับเป็น 2 วันเอง (ถ้า 17 ต.ค. เป็นวันทำงาน)

### Verified
`tests/leave_recalc.test.js` 7 ข้อ — v1.6.11 ตกทั้ง 7 · ชุดเดิม 57 ข้อผ่านเท่าเดิม

---

## [1.6.11] — 2026-09-22 (แปลงวันที่ปี พ.ศ. ที่ค้างอยู่ในคำขอ)

### Fixed
- **วันที่ปี พ.ศ. ที่บันทึกไปก่อน v1.6.8 ถูกแปลงเป็น ค.ศ. อัตโนมัติ** หลังดึงข้อมูลจาก cloud สำเร็จ — ช่อง date / startDate / endDate / workDate / earnedDate / expireDate ของคำขอทั้ง 6 คีย์ · ช่องข้อความไม่แตะ · ไม่มีอะไรให้แปลงก็ไม่บันทึก · จด audit log `be-date-migrate` ต่อคีย์พร้อมรายการที่แปลง
- ผลตรวจ 120: OT อนุมัติแล้ว 18 รายการ 18.5 ชม. (641203001 × 16, 690716001 × 2) ไม่ตกในรอบเงินเดือนไหนเลยเพราะเป็นปี 2569 · ใบลาอนุมัติ 1 ใบ (690519001, 24 ส.ค.) ไม่ถูกหักสิทธิ์/ไม่ตัดเบี้ยขยัน · วันสะสม 1 รายการ (460515001) หมดอายุปี 2570

### ผลที่ตามมา (ต้องแจ้งบัญชี/HR)
- OT 18 รายการตกไปอยู่รอบ มิ.ย.–ส.ค. ที่ผ่านไปแล้ว ถ้าสลิปรอบนั้นถูกปิดในระบบ กลไกยกยอดปรับปรุงจะคำนวณใหม่และยกส่วนต่างเข้ารอบถัดไปเอง ถ้ารอบนั้นจ่ายนอกระบบ ต้องจ่ายย้อนหลังด้วยมือ
- ใบลาของ 690519001 วันที่ 24 ส.ค. จะเริ่มถูกนับ: หักสิทธิ์ลา และอาจตัดเบี้ยขยันรอบ ส.ค. ย้อนหลัง
- วันสะสมของ 460515001 กลายเป็นได้วันที่ 8 ธ.ค. 2026 ซึ่งยังมาไม่ถึง — น่าจะตั้งใจพิมพ์ 2568 ให้ HR ตรวจวันจริง

### Why in the app, not SQL
เครื่องที่ถือสำเนาเก่าจะเขียนปี 2569 กลับขึ้นไปได้ ถ้าแก้ในแอป ทุกเครื่องแปลงสำเนาของตัวเองด้วย · ต้องมี trigger รวมรายการ (db/merge ติดตั้ง 22 ก.ย. 13:xx) ก่อน ไม่งั้นการบันทึกทั้งชุดจากเครื่องที่แปลงอาจทับคำขอใหม่ของคนอื่น

### Verified
`tests/be_migrate.test.js` 5 ข้อ — v1.6.10 ตกทั้ง 5 · ชุดเดิม 52 ข้อผ่านเท่าเดิม

---

## [1.6.10] — 2026-09-22 (ฝั่งแอปของการรวมรายการตามรหัส — กันบันทึกทับกันระหว่างเครื่อง)

**ต้องใช้คู่กับ `db/merge/01_install_merge_trigger.sql`** ลำดับ: push แอปนี้ก่อน → ติดตั้ง trigger ทีหลัง (แอปนี้ทำงานได้ปกติแม้ยังไม่มี trigger)

### ปัญหาที่พบ (22 ก.ย. 2026, ผล 119/121/122)
พนักงาน 2 คนยื่น OT ห่างกัน 0.36 วินาทีจากคนละเครื่อง → ของคนแรกหาย แอปส่งรายการคำขอ **ทั้งชุด** จากเครื่องตัวเองขึ้นไปแทนที่ มีแค่ `employees` ที่ดึงของในระบบมารวมก่อน การลบ 1 เพิ่ม 1 ทำให้จำนวนเท่าเดิม guard ≥3 จับไม่ได้ · เป็นกลไกเดียวกับที่ทำให้คำขอหาย/อนุมัติถอยกลับเมื่อ ส.ค.

### Added
- **`DB.markDeleted(key, ids)`** — บันทึกการลบจริงไว้ใน `deletedRecordIds` ก่อนบันทึกรายการ เรียกที่จุดลบทั้ง 8 จุด (ลบลา/OT/สะสมวันหยุด/นอกสถานที่/รับรองเวลา, เพิกถอนวันสะสม, ลบพนักงานแบบยกชุด) · `DB.save()` รอให้บันทึกการลบถึงฐานข้อมูลก่อนค่อยส่งรายการ
- **รับค่าที่ฐานข้อมูลรวมแล้วกลับมาเก็บในเครื่อง** — คีย์คำขอ 6 คีย์ขอ `select('value')` หลัง upsert เครื่องที่บันทึกจึงเห็นรายการที่เครื่องอื่นเพิ่งเพิ่มทันที (เดิม realtime ของเครื่องอื่นถูกข้ามระหว่างที่เรามีการบันทึกค้าง แล้วไม่ถูกเล่นซ้ำ)
- ถ้ารายการที่เครื่องนี้แก้ถูกฐานข้อมูลคงค่าล่าสุดไว้ (เพราะแก้จากสำเนาเก่า) → เตือนผู้ใช้ ไม่เงียบ

### Design note
ตั้งใจ **ไม่** หาการลบจากผลต่างกับค่าที่เก็บไว้ในเครื่อง: ถ้าหน้าจอถือ array เก่าอยู่ตอนที่ realtime เอารายการใหม่ของเครื่องอื่นมาใส่ รายการนั้นจะดูเหมือนถูกลบ แล้ว trigger จะลบทิ้งจริง — เปลี่ยนเหตุการณ์ที่กู้ได้ให้เป็นการลบถาวร การลบจึงบันทึกเฉพาะที่จุดลบแต่ละจุด และมีเทสต์ที่ไล่ทุก `DB.save(คีย์คำขอ, x.filter(...))` ในโค้ดว่ามี `markDeleted` นำหน้า

### Verified
`tests/merge_client.test.js` 6 ข้อ (supa จำลอง) — v1.6.9 ตกทั้ง 6 · ชุดเดิม 46 ข้อผ่านเท่าเดิม · trigger ฝั่งฐานข้อมูลยังไม่ได้ทดสอบกับ Postgres จริง

---

## [1.6.9] — 2026-09-22 (ปฏิทินทีม · ปฏิทินของฉันตรงกับสลิป · ใบรับรองเวลารั่วเข้าสลิปคนอื่น)

### Security / Money — Fixed
- **สลิปเงินเดือนรับใบรับรองเวลาของคนอื่นมาเป็นของตัวเอง** — `mergeApprovedTimeCertForDate()` ถ้าได้รายการสแกนว่าง จะใส่ใบรับรองเวลาของ *ทุกคน* ในวันนั้นเข้ามา (ตั้งใจไว้ให้รายงานรายวัน) คนที่ไม่มีสแกนเลยทั้งรอบ — พนักงานใหม่ ลาคลอด คนที่ละเว้นการสแกน — จึงได้เวลาของเพื่อนร่วมงานไปคิดสาย/ขาดงาน และ**ถูกหักเงินจากเวลาของคนอื่น** ตั้งแต่ v1.6.0 ที่เริ่มหักขาดงาน แก้ที่ `calculatePaySlip` ให้กรองเหลือเฉพาะของคนนั้นหลัง merge · ทดสอบ: v1.6.8 หัก E1 หนึ่งวันจากใบรับรองของ E2

### Added
- **แถบ "ปฏิทินทีม" ในหน้ารายงานทีม (Manager) และหน้ารายงาน (Admin)**
  - ขอบเขตเดียวกับศูนย์อนุมัติ: Manager เห็นลูกน้องที่ตัวเองเป็นผู้อนุมัติ · Admin เห็นทุกคน และกรองตามบริษัท/แผนกได้
  - มุมมองทั้งทีม: แต่ละวันบอกจำนวน 🟢 มาทำงาน · 🔴 สาย · 🟠 ลา · ❌ ขาด กดวันไหนก็เห็นรายชื่อพร้อมกะ สถานะ และเวลาเข้า-ออก (เรียงคนขาด/สายขึ้นก่อน)
  - เลือกพนักงานทีละคน: เห็นเป็นปฏิทินแบบเดียวกับ "ปฏิทินของฉัน" ของคนนั้น

### Fixed — ปฏิทินของฉันไม่ตรงกับสลิปเงินเดือน
สถานะรายวันย้ายไปอยู่ใน `buildCalendarCtx()` + `calendarDayStatus()` ใช้ร่วมกันทั้งสองปฏิทิน ลำดับเดียวกับ `isNonWorkingDay()` และ `calculatePaySlip()`
- วันทำงานนอกสถานที่ที่อนุมัติแล้ว เดิมขึ้น "ขาดงาน" → ขึ้น 🧳 นอกสถานที่
- สแกนเกินเกณฑ์ขาดงานของกะ เดิมขึ้นเป็นจุดแดง (สาย) ทั้งที่สลิปหักเป็นขาดงาน → ขึ้น ❌ ขาดงาน พร้อมเวลาที่สแกน
- วันทำงานดูจากตารางกะ ไม่ใช่เสาร์-อาทิตย์ — คนที่มีกะวันเสาร์ไม่สแกนขึ้นขาดงาน · วันหยุดบริษัทที่มีกะทำงานนับเป็นวันทำงาน
- ใบรับรองเวลาวันที่ 22 เป็นต้นไปไม่แสดง เพราะดึงตามรอบเงินเดือนแทนเดือนปฏิทิน
- ใบรับรองเวลาของคนอื่นโผล่ในปฏิทินของคนที่ไม่มีสแกนทั้งเดือน (สาเหตุเดียวกับบั๊กสลิปด้านบน)
- วันที่ยังไม่ได้อัปโหลดไฟล์สแกน เดิมขึ้น "ขาดงาน" → ขึ้น "รอข้อมูลสแกน" (ดูจากวันล่าสุดที่มีไฟล์สแกนของทั้งบริษัท)

### Fixed — หน้าตา
- ปฏิทินล้นขอบบนมือถือ วันเสาร์และตัวเลขวันที่ถูกตัด — `aspect-ratio: 1` + `min-height: 90px` บังคับความกว้างขั้นต่ำ 90px ต่อช่อง (7 ช่อง = 630px) แก้ด้วย `width:100%` + `minmax(0,1fr)` จอกว้างยังเป็นช่องจัตุรัสเหมือนเดิม
- ข้อความสีในช่องวันอาทิตย์และวันนี้ อ่านไม่ออกบนพื้นน้ำเงินเข้ม → ใช้สีของช่อง

### Known / ไม่ได้เปลี่ยน
- สลิปหัก "ขาดงาน" เฉพาะวันที่สแกนเกินเกณฑ์ วันที่ไม่มีสแกนเลยไม่ถูกหัก ปฏิทินแสดงทั้งสองแบบเป็น "ขาดงาน" เพื่อให้หัวหน้าเห็น — พฤติกรรมสลิปคงเดิม
- `resolveAttendanceStatus()` (ADR-0002) ไม่มีที่ไหนเรียกใช้ และจัด `sick-with-cert` เป็น "ลาอื่นๆ" — ไม่ได้แตะ

### Verified
- `tests/calendar.test.js` 8 ข้อ — v1.6.8 ตกทั้ง 8 (รวมข้อสลิปหักเงินจากใบรับรองของคนอื่น) · ชุดเดิม 38 ข้อผ่านเท่าเดิม
- ปฏิทินทีมเปิดในเบราว์เซอร์จริงด้วยโค้ดที่ดึงจาก index.html + ข้อมูลสมมติ (ไม่ login แอปจริง ไม่แตะ Supabase): มุมมองทั้งทีม · รายวัน · รายคน · Admin กรองบริษัท/แผนก · มือถือ 375px ไม่ล้น · จอ 1280px ช่องจัตุรัสเท่าเดิม
- "ปฏิทินของฉัน" ใช้ตัวคำนวณและตัววาดช่องเดียวกันที่ทดสอบแล้ว แต่ไม่ได้เปิดดูหน้าจริงในเบราว์เซอร์

---

## [1.6.8] — 2026-09-19 (กันปี พ.ศ. ทุกช่องวันที่ และไฟล์นำเข้า)

v1.6.7 กันเฉพาะวันเริ่มงาน แต่ทุกช่องวันที่มีปัญหาเดียวกัน — พิมพ์ 2569 แล้วระบบเก็บเป็นปี ค.ศ. 2569 โดยไม่มี error: ใบลาไม่ถูกนับในปีไหนเลย วันหยุดบริษัทไม่มีผล OT/ใบรับรองเวลาไม่ตกในรอบเงินเดือน

### Fixed
- **ทุกช่องวันที่และช่องเดือนแปลงปี พ.ศ. เป็น ค.ศ. ให้อัตโนมัติ** แล้วแจ้งว่า "แปลงปี พ.ศ. 2569 เป็น ค.ศ. 2026 ให้แล้ว" — ครอบคลุม 10 ช่องวันที่ (ลา, OT, ใบรับรองเวลา, ทำงานวันหยุด, ทำงานนอกสถานที่, วันเริ่มงาน, ช่วงรายงาน, วันหยุดบริษัท) และ 5 ช่องเดือน (ตารางกะ, คอมมิชชั่น, สลิป) รวมถึงช่องที่จะเพิ่มในอนาคต เพราะดักครั้งเดียวที่ระดับหน้า ไม่ได้ผูกทีละช่อง
- ปี 2400 ขึ้นไปถือเป็น พ.ศ. แน่นอน จึงแปลงเลยไม่ต้องบล็อก · ปีที่อยู่ในอนาคตเกิน 1 ปีแต่ต่ำกว่า 2400 (พิมพ์ผิด เช่น 2062) แค่เตือน ไม่แปลง
- 29 ก.พ. ของปี พ.ศ. ที่ไม่มีในปี ค.ศ. ที่แปลงได้ → ล้างช่องและบอกให้เลือกใหม่ แทนการแปลงเป็นวันอื่นเงียบๆ
- **ไฟล์เข้างานจากเครื่องสแกน/Excel ที่เป็นปี พ.ศ.** (`25/08/2569`) แปลงเป็น ค.ศ. ตอนนำเข้า — เดิมผ่านไปเป็นปี 2569 ตรงๆ รายการนั้นจึงไม่ตกในรอบเงินเดือนไหนเลย
- ข้อความเตือนวันเริ่มงานของ v1.6.7 ไม่แนะนำ "ค.ศ. 1519" กับปีที่พิมพ์ผิดแบบ 2062 อีก

### Verified
- `tests/be_year.test.js` 6 ข้อ — v1.6.7 ตก 5 (ผ่านเฉพาะข้อ "ปี ค.ศ. เหมือนเดิม") · ชุดเดิม 32 ข้อผ่านเท่าเดิม
- ทดสอบพิมพ์จริงในเบราว์เซอร์ด้วยโค้ดที่ดึงจาก index.html: พิมพ์ปี 2569 ทีละตัว → ค่าระหว่างพิมพ์ (0002, 0025, 0256) ไม่ถูกแตะ · พอครบ 2569 แปลงเป็น 2026 และ onchange ของช่องเห็นค่าที่แปลงแล้ว (ดักแบบ capture ทำงานก่อน)
- ช่องเดือนทดสอบเฉพาะตัวแปลง ไม่ได้ทดสอบพิมพ์จริง

---

## [1.6.7] — 2026-09-19 (กันวันเริ่มงานที่กรอกเป็นปี พ.ศ.)

### Fixed
- **ฟอร์มพนักงานไม่รับวันเริ่มงานที่อยู่ในอนาคตเกิน 1 ปี** และแนะนำปี ค.ศ. ที่ถูกให้ — ช่องวันที่รับปี ค.ศ. แต่พิมพ์ 2569 ลงไปได้ ระบบเลยเข้าใจว่าเริ่มงานในอีก 543 ปี พนักงานคนนั้นติดทดลองงานตลอดไป ลาพักร้อนเป็น 0 และเลือกลาพักร้อนไม่ได้ โดยไม่มี error ให้เห็น พบ 2 คนในข้อมูลจริง (อายุงาน −543 ปีในผลตรวจ 112) · เงินเดือนไม่กระทบ เพราะสูตรสลิปไม่ได้ใช้วันเริ่มงาน
- ข้อมูล 2 คนเดิมยังต้องแก้ผ่านฟอร์ม ตัวกันนี้ป้องกันเฉพาะการบันทึกครั้งต่อไป

### Confirmed (ไม่มีการเปลี่ยนโค้ด)
- ผลตรวจ 112: การเพิ่มวันลาตามอายุงาน **ปิดอยู่** บนระบบจริง นโยบายจริงคือ 6 วันทุกคน ซึ่งตรงกับค่า `vacationQuota` = 6 ของ 134 คน หน้าประกาศนโยบายแสดง "6 วัน/ปี" ถูกต้อง
- ไม่ย้อนยกยอดวันลาปี 2025 → 2026 ตามที่ตั้งไว้ใน v1.6.6

---

## [1.6.6] — 2026-09-19 (สิทธิ์ลาพักร้อน: ได้เต็มเมื่อผ่านทดลองงาน · ยกยอดข้ามปี)

### Changed
- **ผ่านทดลองงานแล้วได้ลาพักร้อนเต็มจำนวนทันที ไม่คิดตามสัดส่วน** — เดิมคิดตามเดือนที่เหลือของปีที่เข้าทำงาน: เข้า 1 มี.ค. ได้ 4 วัน ผ่านทดลองงานเดือน ธ.ค. ได้ 0.5 วัน แล้วทุกคนได้ 6 วันพร้อมกันวันที่ 1 ม.ค. ระหว่างทดลองงานยังเป็น 0 เหมือนเดิม (มาตรา 30 กำหนดขั้นต่ำ 6 วันเมื่อทำงานครบ 1 ปี การให้ก่อนครบปีจึงใจดีกว่ากฎหมาย)

### Added
- **ยกยอดลาพักร้อนข้ามปี** ตามที่หน้าตั้งค่าและหน้าประกาศนโยบายบอกพนักงานมาตลอด ("ยกได้สูงสุด 5 วัน ตัดยอด 31 มี.ค.") แต่สูตรไม่เคยใช้ค่านี้ วันที่เหลือจึงหายทุก 1 ม.ค.
  - ยกได้ไม่เกิน `leaveCarryOverMax` วัน ใช้ได้ถึง `leaveCarryOverCutoff`
  - ลาที่เริ่มก่อนวันตัดยอด หักจากยอดยกมาก่อน (FIFO) · พ้นวันตัดยอดแล้วส่วนที่ไม่ได้ใช้หมดอายุ ส่วนที่ใช้ไปแล้วไม่ย้อนกลับมาเป็นหนี้
  - ตอนคิดยอดเหลือของปีก่อน หักส่วนที่ปีก่อนยกมาออกก่อน — คนที่ใช้ยอดยกมาจนหมดจะไม่ถูกนับว่าใช้สิทธิ์ของปีตัวเองเกิน
  - **ไม่ยกยอดให้คนที่ HR แก้โควตาเองรายคน** — ค่านั้นคือสิทธิ์คงเหลือจริงที่ HR ตั้งไว้ ถ้าบวกยอดยกมาอีกจะนับซ้ำ
  - **เริ่มยกจากปี 2026 เข้าปี 2027** (`leaveCarryOverFromYear`) ไม่ย้อนคิด 2025 → 2026 เพราะยอดคงเหลือปีนี้ที่พนักงานเห็นอยู่และใช้ตัดสินใจลาไปแล้วจะเปลี่ยนกลางปี ถ้าต้องการย้อนหลังให้ตั้งเป็น 2025
  - หน้าแรกของพนักงานบอกยอดยกมาที่ยังเหลือและวันหมดอายุ
- `getLeaveBalance().vacation` คืน `carryOver`, `carryUsed`, `carryActive`, `carryExpires`, `carryRemaining` เพิ่ม

### Fixed
- **แก้ข้อมูลพนักงานแล้วโควตาลาพักร้อนถูกรีเซ็ตเป็น 6** — ฟอร์มพนักงานใส่ `vacationQuota: 6` ไว้ในก้อนข้อมูลที่ถูก spread ทับข้อมูลเดิมตอนบันทึก การแก้ช่องไหนก็ตาม (เบอร์โทร แผนก สิทธิ์) จึงเขียนทับค่าที่ตกลงไว้กลับเป็น 6 ตอนนี้ตั้งค่า 6 เฉพาะตอนสร้างพนักงานใหม่ ค่าที่ HR ตั้งผ่านหน้าแก้โควตา (`leaveQuotaOverride`) ไม่เคยโดนผลกระทบนี้ เพราะเป็นอีกฟิลด์และมีลำดับก่อน

### Known consequence (ตั้งใจ)
ผ่านทดลองงานเดือน ธ.ค. ได้ 6 วันทันที ถ้าไม่ได้ใช้ ต้นปีถัดไปมีได้ถึง 11 วัน (6 + ยกมา 5) จนถึง 31 มี.ค.

### Verified
`tests/vacation.test.js` 13 ข้อ — บิลด์ v1.6.5 ตก 12 ข้อ (ผ่านเฉพาะ "ระหว่างทดลองงาน = 0" ที่พฤติกรรมไม่เปลี่ยน) · ชุดสลิปเงินเดือน 12/12 และขาดงานตามกะ 7/7 ผ่านเท่าเดิม

---

## [1.6.5] — 2026-09-19 (ขาดงานอัตโนมัตินับจากเวลาเข้ากะของแต่ละคน)

**"สาย" ดูตารางกะรายวันมาตั้งแต่ v1.5.4 แต่ "ขาดงานอัตโนมัติ" ใช้เวลากลางของบริษัท** — 09:01 + 240 นาที = 13:01 กับทุกคนทุกวัน

| กะ | สแกน | ควรเป็น | ระบบเดิมบอก |
|---|---|---|---|
| 13:00 | 13:05 | สาย 5 นาที | **ขาดงาน** |
| 11:00 | 13:30 | สาย 2.5 ชม. | **ขาดงาน** |
| 14:00 | 13:55 | ปกติ | **ขาดงาน — ทุกวันที่มาทำงาน** |

**ตั้งแต่ v1.6.0 ขาดงานถูกหักเท่าค่าแรงหนึ่งวัน บั๊กนี้จึงกลายเป็นเงินจริง** — คนเข้ากะบ่ายโมงที่สาย 5 นาทีเสียค่าแรงทั้งวันแทนค่าปรับสาย และคนที่กะเริ่มหลัง 13:01 ถูกหักทุกวัน การหักขาดงานใน v1.6.0 ทำให้บั๊กที่เดิมเป็นแค่ป้ายผิดในรายงาน กลายเป็นการหักเงินผิด

### Fixed
- **เกณฑ์ขาดงานนับจากเวลาเข้ากะของคนนั้นในวันนั้น** ผ่าน `computeAutoAbsentCutoffFor()` ตัวใหม่ ใช้ทั้งรายงานเข้างานและสลิปเงินเดือน — กะ 13:00 ขาดงานเมื่อสแกนหลัง 17:01 ไม่ใช่ 13:01
- **วันหยุดตามกะไม่มีการขาดงาน**
- **วันที่ไม่ได้ลงกะไว้ ใช้เวลากลางของบริษัทตามเดิม** — แปลว่าคนที่เข้ากะบ่ายเป็นประจำ ต้องลงกะไว้ในตารางทุกวัน ไม่งั้นระบบประเมินเขาแบบคนเข้า 9 โมง
- ข้อความบนการ์ดรายงานและหน้าตั้งค่าที่เขียนเวลาตายตัว แก้ให้บอกว่านับจากกะของแต่ละคน

### Design note
ตั้งใจไม่ใช้ `addMinutesToTimeStr()` ที่มีอยู่แล้ว เพราะมันวนรอบเที่ยงคืนด้วย `% 24` — กะ 21:00 จะได้เกณฑ์ขาดงานเป็น 01:01 แล้วทุกการสแกนจะ ≥ 01:01 ถูกนับขาดงานทั้งหมด ตัวใหม่ตัดที่ 23:59 เหมือนตัวเดิม

### Verified
7 assertions เทียบกับ v1.6.4 — control ตกเฉพาะ 4 ข้อที่แก้ (กะ 13:00, 11:00, 14:00, 21:00) และผ่าน 3 ข้อที่ยืนยันพฤติกรรมเดิม (กะ 9 โมง, ขาดงานจริงเกิน 4 ชม., วันไม่ลงกะ) · ชุดทดสอบสูตรเงินเดือน 12 ข้อผ่านเท่ากันทั้งสองบิลด์

ชุดทดสอบย้ายจาก scratchpad มาไว้ที่ `tests/` ในโปรเจกต์ เพราะชุดเดิมหายไปตอน scratchpad ถูกล้างระหว่างเซสชัน — `tests/` ถูก gitignore ไว้แล้ว จึงไม่ขึ้น repo สาธารณะ
